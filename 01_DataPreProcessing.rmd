...
3. Handling missing data and sample weighting.
4. Selecting and creating the outcome variables (i.e., college enrollment and college completion).
...
list.of.packages <- c("foreign", "psych")
...
```{r}
#youth report
Ymomeduc1<-as.numeric(substr(wave1$H1RM1, start = 2, stop = 3)) 
Ymomeduc1[which(Ymomeduc1==10)]<-1 #never went to school
Ymomeduc1[which(Ymomeduc1==9)]<-8
Ymomeduc1[which(Ymomeduc1==11|Ymomeduc1==12)]<-NA
mean(Ymomeduc1, na.rm = TRUE)
length(spss1[as.numeric(spss1$H1RM1)==14,]$AID) #legitimate skip
Ymomeduc1[which(as.numeric(spss1$H1RM1)==14)]<- 5.435109 #no resident mom
summary(as.factor(Ymomeduc1))


Ydadeduc1<-as.numeric(substr(wave1$H1RF1, start = 2, stop = 3)) 
Ydadeduc1[which(Ydadeduc1==10)]<-1 #never went to school
Ydadeduc1[which(Ydadeduc1==9)]<-8
Ydadeduc1[which(Ydadeduc1==11|Ydadeduc1==12)]<-NA
mean(Ydadeduc1, na.rm = TRUE)
length(spss1[as.numeric(spss1$H1RF1)==14,]$AID) #legitimate skip
Ydadeduc1[which(as.numeric(spss1$H1RF1)==14)]<- 5.523199 #no resident mom
summary(as.factor(Ydadeduc1))

dat1$momeduc<-Ymomeduc1
dat1$dadeduc<-Ydadeduc1
```
...
Parent presence at evening meal
*no parent = mean
```{r}
levels(spss1$H1WP8)
...
DadObese[which(HasDad==0)]<-0.1414504
...
```{r}
Wave1Vars<-dat1
write.csv(Wave1Vars, "Wave1Vars.csv", row.names = FALSE)
Wave1Vars<-read.csv("Wave1Vars.csv")
```


weights at wave IV
```{r}
load("Wave4Weights.rda") #Use Wave IV weights instead! See example w/ Wave III data: Perreira, Krista M.; Harris, Kathleen Mullan; & Lee, Dohoon. (2006).
Wave4Weights<-da21600.0031
Wave4Weights_c<-Wave4Weights[c('AID', 'GSWGT4_2')]
dat14<-merge(Wave1Vars, Wave4Weights_c, by = 'AID')
```
...
collcom_di[which(atschool==1 & education==6)]<-NA
...
cols<-c("Mother" , "Father", "TwoBioParent", "PAassistance", "welfare", "PAecohard", "PAPTA", "mnativity", "dnativity" , "PAsmoke", "malcohol", "dalcohol", "mobese", "dobese"  ,"mdisable" , "ddisable", "HHsmoke", "HHdrug", "EnglishHome", "biosex", "Latino", "AA", "Native", "Asian", "other_race", "nativity" )
...
For parsimony given two algorithms and nested cross-validation to run,
here just take the mean or majority vote

```{r}
dat1_re3<-read.csv("dat1_re3.csv")
dat1_re3_mi<-dat1_re3
```

```{r}
factorname<-c("Mother" , "Father", "TwoBioParent", "PAassistance", "welfare", "PAecohard", "PAPTA", "mnativity", "dnativity" , "PAsmoke", "malcohol", "dalcohol", "mobese", "dobese"  ,"mdisable" , "ddisable", "HHsmoke", "HHdrug", "EnglishHome", "biosex", "Latino", "AA", "Native", "Asian", "other_race", "nativity" )
for(j in 2:54){
  for(i in 1:4598){
